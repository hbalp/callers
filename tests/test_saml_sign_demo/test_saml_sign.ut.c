...
int main()
{
  ...
  printf("XML signature validation with XSW countermeasure:\n-----\n");
#else
  printf("Verify XML signature without XSW countermeasure:\n-----\n");
#endif  
  printf("Verify XML signature in nominal case...\n");
  status = status && ut_saml_SignatureProfileValidator_validate(false);
  printf("Verify XML signature under XSW attack...\n");
