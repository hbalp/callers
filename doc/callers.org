...
** Analysis results and exploitation [/]
   ...
   This is the purpose of the "function_calls_to_dot" backend implemented in OCAML and used in step 7.
...
Then you have to make sure that all system packages used by the callers are well installed on the host, like:
 + rapidjson-dev
...
* References
  ...
  | scan-build                      | http://clang-analyzer.llvm.org/scan-build.html   |
   #+END_SRC
* TODO Todo list [/]
** Support for inheritance and virtualization [3/7]
*** DONE json file extension [3/3]
    1. [X] add a *record* type (record=struct|class|union)
    2. [X] add a *virtuality* type 
    3. [X] add an *inheritance* type
*** extended json formats in the reference example
*** TODO evolutions of the C++ frontend [3/4]
**** DONE step 0. _to add record definition (required for frontend, optional for backend)_
***** in file A.hpp.json
...
**** DONE step 1. _to produce the virtuality flag for each virtual method declaration_
       ...
       Add a flag "virtuality" to be used only in case of virtual method
       "virtuality": "no | pure | declared | defined"
...
****** extended json file content
       {
         ...
         "defined": [ 
            ...
            { "sign": "int c()", "line": 17, "virtuality": "pure" } 
          ]
       }

**** DONE step 2. _build the inheritance graph in json format_
         ...
         "records": [
           {
               ...
               "inherits": [ 
                   { 
                     "C",
                     "decl":
                     "/home/hbalp/work/src/llvm/trunk/tools/clang/tools/extra/callers/tests/test_local_callgraph_several_files/dirC/C.hpp:7:14"
                  }
                ]
           }
          ],
...
**** TODO step 3. _to produce or not the virtual flag at the caller site, we need to_
...
****** extended json file content
         {
           ...
           "decl":
             "/home/hbalp/work/src/llvm/trunk/tools/clang/tools/extra/callers/tests/test_local_callgraph_several_files/dirA/A.hpp:17",
           "def": "unlinkedExtCallee"
           "virtuality":"pure|declared|defined"
         }
       	
*** DONE Add the *add_virtual_function_calls* OCAML backend [1/1]

    1. [X] link: visits all virtual methods
       - [X] for each virtual method, add extcallees for each derived method
             +browse all callers of those virtual methods and replace each original function call with a list of function call, one per derived method+
             +replaces the mother class name by the child class name in methods to link the caller and callee.+

*** DONE [#A] Add an OCAML backend to build class inheritance graphs
    DEADLINE: <2015-10-22 jeu.>
**** DONE First begin by listing all records without looking at the inheritance links
**** DONE Then add the *inheritance* links
**** DONE Add the *add_inherited* OCAML backend to complete declared base classes with their list of children's classes
**** DONE Add an *classes_to_dot* OCAML backend to generate class diagrams
*** TODO Add a declaration field to defined function
    For enabling navigation from a *method* towards the declaration of the class
    exemple: method ::Newly::Added::D::c() of test_local_callgraph_several_files
*** TODO Add the *add_definitions* OCAML backend
*** TODO Add the *add_redefinitions* OCAML backend
** Adding C++ mangling information
    DEADLINE: <2016-01-18 lun.>
   1. [ ] complete local info with caller_mangled in methods [/]
      1. [ ] void CallersData::FctDecl::add_local_caller
      2. [ ] complete local info with caller_mangled in method void CallersData::FctDecl::add_local_callee
...
* COTS
  ...
  $ clang --version
  clang version 3.8.0 (trunk 254623) (llvm/trunk 254621)
  Target: x86_64-unknown-linux-gnu
  Thread model: posix

  # Installed packages for 4.02.0:
  atd                1.1.2  Parser for the ATD data format description language
  atdgen             1.7.1  Generates efficient JSON serializers, deserializers and validators
  ...
  core           112.01.01  Industrial strength alternative to OCaml's standard library
  core_kernel    112.01.00  Industrial strength alternative to OCaml's standard library
  ...
  easy-format        1.1.0  High-level and functional interface to the Format module of the OCaml standard library
  enumerate      111.08.00  Quotation expanders for enumerating finite types.
  fieldslib      109.20.03  Syntax extension to define first class values representing record fields, to get and set record fields, iterate and fold over all fields of a record and create new re
  ...
  type_conv      112.01.02  Library for building type-driven syntax extensions
