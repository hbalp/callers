...
"metadata json file" -> "callers backends" [ dir=back, color=green ]
...
"metadata json file" -> "callers backends" [ dir=back, color=blue ]
...
rm -f *.gen.*
     #+END_SRC
**** TODO [#A] Envoyer ces figures à Frank Védrine comme annoncé hier :email:cea:
